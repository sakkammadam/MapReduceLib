...
public:
    ...
    Mapper(const std::map<std::string, std::vector<std::vector<std::string>>> &processed_directory): MapperBase(processed_directory){
...
extern "C" MapperBase* createInputObj(const std::map<std::string, std::vector<std::vector<std::string>>> &processed_directory){
    return new Mapper(processed_directory);
}
