...
public:
    ...
    FileProcessorRedOutput(
            ...
            const std::vector<std::map<std::string, std::map<std::string, size_t>>> &reduced_result
            ):FileProcessorBase(){
        // Sets the operation!
    ...
    virtual void runOperation() {
        ...
        if (this->getOperation() == "reducer"){
            ...
            // Reducer data
            std::vector<std::map<std::string, std::map<std::string, size_t>>> reduceData = this->getRawReducerOutput();
            // Shuffle parent path
